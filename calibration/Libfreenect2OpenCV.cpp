...
void Libfreenect2OpenCV::updateMat()
{
    ...
    while (!s_shutdown)
    {
      ...
      //  cv::resize(m_rgbMat, m_rgbMat, cv::Size(m_rgbMat.cols / 4, m_rgbMat.rows / 4));
        ...
        cv::Mat depthMatUndistorted_tmp(undistorted.height, undistorted.width, CV_32FC1, undistorted.data);
        cv::flip(depthMatUndistorted_tmp, m_depthMatUndistorted,1);

        m_depthMatUndistorted /= 4096.0f;

        cv::Mat rgbd_tmp(registered.height, registered.width, CV_8UC4, registered.data);
        cv::flip(rgbd_tmp,m_rgbdMat,1);

        cv::Mat big_depth(depth2rgb.height, depth2rgb.width, CV_32FC1, depth2rgb.data);
        cv::flip(big_depth,m_rgbd2Mat,1);
